...
import { MoreHorizontal, MicOff, Volume2, VolumeX, X, MonitorOff, Repeat, EyeOff, Maximize2, HeadphoneOff } from 'lucide-react';
...
const VideoTile = memo(function VideoTile({
  ...
  hideControls = false, deafened = false
}: VideoTileProps) {
  const { t } = useTranslation();
  const [showOptions, setShowOptions] = useState(false);
  const [volValue, setVolValue] = useState(Math.round(volume * 100));
  ...
  useEffect(() => {
    ...
    const close = (e: MouseEvent) => {
      if ((e.target as HTMLElement).closest('.options-menu-container')) return;
  ...
  return (
    <div
      ...
      style={{
        ...
        boxShadow: speaking ? '0 0 15px rgba(35,165,90,0.4)' : '0 4px 15px rgba(0,0,0,0.2)',
        transition: 'all 0.2s cubic-bezier(0.4, 0, 0.2, 1)', cursor: onClick ? 'pointer' : 'default',
        ...style
      }}
    >
      ...
      <div style={{ position: 'absolute', bottom: '12px', left: '12px', zIndex: 10, display: 'flex', alignItems: 'center', gap: '8px' }}>
        <div style={{ backgroundColor: 'rgba(0,0,0,0.6)', backdropFilter: 'blur(12px)', padding: '4px 10px', borderRadius: '4px', color: '#fff', fontSize: '11px', fontWeight: 600, display: 'flex', alignItems: 'center', gap: '6px' }}>
          {muted && !deafened && !sharing && <MicOff size={11} style={{ color: '#ed4245' }} />}
          {deafened && !sharing && <HeadphoneOff size={11} style={{ color: '#ed4245' }} />}
          {displayName} {isLocal && !sharing && `(${t('common.you', 'Tú')})`}
          {sharing && <span style={{ color: '#23a55a', fontWeight: 800 }}>LIVE</span>}
        </div>
      </div>

      {(!isLocal || (sharing && isLocal)) && !hideControls && (
        <button
          onClick={toggleOptions}
          style={{ position: 'absolute', top: '12px', right: '12px', zIndex: 20, width: '32px', height: '32px', borderRadius: '50%', border: 'none', backgroundColor: 'rgba(0,0,0,0.4)', color: '#fff', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', backdropFilter: 'blur(4px)', transition: 'transform 0.2s' }}
