      ...
      <div style={{ width: '100%', maxWidth: 500, height: 700, backgroundColor: colors.background, borderRadius: 24, display: 'flex', flexDirection: 'column', overflow: 'hidden', boxShadow: '0 20px 50px rgba(0,0,0,0.5)' }}>
        <div style={{ padding: '20px 24px', borderBottom: `1px solid ${colors.border}`, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            ...
            <ThemedText style={{ fontSize: 18, fontWeight: '900' }}>Gestionar Usuarios</ThemedText>
          </div>
        ...
        <div style={{ padding: 16, display: 'flex', flexDirection: 'column', gap: 12, borderBottom: `1px solid ${colors.border}` }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 14px', backgroundColor: colors.backgroundSecondary, borderRadius: 12 }}>
            ...
            <input placeholder="Buscar por nombre o email..." value={search} onChange={e => setSearch(e.target.value)} style={{ flex: 1, background: 'none', border: 'none', outline: 'none', color: colors.text, fontSize: 14 }} />
          </div>
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: 10 }}>
          {loading ? (
            <div style={{ display: 'flex', justifyContent: 'center', padding: 40 }}><ThemedText>Cargando...</ThemedText></div>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '12px 16px', borderRadius: 16, backgroundColor: editingUser?.uid === user.uid ? colors.backgroundSecondary : 'transparent' }}>
                ...
                <div style={{ flex: 1 }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                    <ThemedText style={{ fontSize: 12, color: colors.primary, fontWeight: '600' }}>{ROLE_LABELS[user.role || 'student']}</ThemedText>
                    {user.subrole && (
                      <>
                        <div style={{ width: 3, height: 3, borderRadius: 1.5, backgroundColor: colors.textSecondary, opacity: 0.5 }} />
                        <ThemedText style={{ fontSize: 12, color: colors.textSecondary }}>{SUBROLE_LABELS[user.subrole]}</ThemedText>
                      </>
                    )}
                  </div>
                </div>
              ...
              {editingUser?.uid === user.uid && (
                <div style={{ padding: '0 16px 16px 72px', display: 'flex', flexDirection: 'column', gap: 12 }}>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                    <ThemedText style={{ fontSize: 11, fontWeight: '800', color: colors.textSecondary, textTransform: 'uppercase' }}>Cambiar Rol</ThemedText>
                  ...
                  {user.role === 'student' && (
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                      <ThemedText style={{ fontSize: 11, fontWeight: '800', color: colors.textSecondary, textTransform: 'uppercase' }}>Asignar Sub-rol</ThemedText>
