...
export default function ChatScreen() {
  ...
  const isStudyGroup = id?.startsWith('sg_');
  ...
  // Subscribe to active conference for study groups
  useEffect(() => {
  ...
  const handleVotePoll = async (messageId: string, optionId: string) => {
    if (!currentUser || !cleanId) return;
    try {
      const msgRef = doc(db, 'channels', cleanId, 'messages', messageId);
  ...
  return (
    <ThemedView style={{ flex: 1, display: 'flex', flexDirection: 'row', height: '100%', backgroundColor: colors.background, overflow: 'hidden' }}>
      ...
      <div style={{
        ...
        <div 
          onClick={() => {
            if (channelInfo.viewType !== 'events') {
          ...
          style={{ 
            ...
            alignItems: 'center',
            gap: 12,
            cursor: channelInfo.viewType !== 'events' ? 'pointer' : 'default',
            overflow: 'hidden',
            paddingRight: 12
          }}
        >
          ...
          <div style={{ display: 'flex', gap: 4, alignItems: 'center' }}>
            ...
            <button
              ...
              onClick={async () => {
                ...
                const callType = 'video'; // Default to video for conferences
      ...
      {showSettings && (
        <div style={{
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: spacing.md }}>
            <ThemedText style={{ fontWeight: 'bold' }}>{t('chat_ui.customize_title')}</ThemedText>
