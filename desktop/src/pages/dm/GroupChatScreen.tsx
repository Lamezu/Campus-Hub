...
import type { Message, GroupConversation, ReplyPreview, DirectMessage } from '@/types';
...
export default function GroupChatScreen() {
  ...
  const handleSendMessage = async (text: string) => {
    ...
    setReplyingTo(null);
    try {
      const msgId = await groupService.sendGroupMessage(
        groupId, 
        currentUser.uid, 
        ...
        group.members.forEach(uid => {
          if (uid !== currentUser.uid) {
            notificationService.addNotification(uid, {
              title: group.name || 'Chat de grupo',
              body: `${currentUser.displayName || 'Usuario'}: ${text}`,
              category: 'dm',
              meta: { groupId },
            }, msgId);
          }
  ...
  const handleSendAudio = async (url: string, duration: number) => {
    ...
    setSending(true);
    try {
      const msgId = await groupService.sendGroupAudio(
        groupId, 
        currentUser.uid, 
        currentUser.displayName || 'Usuario', 
        currentUser.photoURL, 
      ...
      if (group) {
        group.members.forEach(uid => {
          if (uid !== currentUser.uid) {
            notificationService.addNotification(uid, {
              title: group.name || 'Chat de grupo',
              body: `🎤 Mensaje de voz de ${currentUser.displayName || 'Usuario'}`,
              category: 'dm',
              meta: { groupId },
            }, msgId);
          }
  ...
  const handleSendMedia = async (url: string, type: 'image' | 'video') => {
    ...
    setSending(true);
    try {
      const msgId = await groupService.sendGroupMedia(
        groupId, 
        currentUser.uid, 
        currentUser.displayName || 'Usuario', 
        currentUser.photoURL, 
      ...
      if (group) {
        group.members.forEach(uid => {
          if (uid !== currentUser.uid) {
            notificationService.addNotification(uid, {
              title: group.name || 'Chat de grupo',
              body: `📁 ${type === 'image' ? 'Imagen' : 'Vídeo'} de ${currentUser.displayName || 'Usuario'}`,
              category: 'dm',
              meta: { groupId },
            }, msgId);
          }
  ...
  const handleSendPoll = async (pollData: { question: string; options: string[]; multipleAnswers: boolean }) => {
    ...
    setSending(true);
    try {
      const msgId = await groupService.sendGroupPoll(
        groupId, 
        currentUser.uid, 
        currentUser.displayName || 'Usuario', 
        currentUser.photoURL, 
      ...
      if (group) {
        group.members.forEach(uid => {
          if (uid !== currentUser.uid) {
            notificationService.addNotification(uid, {
              title: group.name || 'Chat de grupo',
              body: `📊 Nueva encuesta de ${currentUser.displayName || 'Usuario'}: ${pollData.question}`,
              category: 'dm',
              meta: { groupId },
            }, msgId);
          }
      ...
      showAlert({ title: 'Error', message: 'No se pudo enviar la encuesta.', type: 'error' });
    } finally {
      setSending(false);
    }
  };
  ...
  if (loading) {
    return (
      <ThemedView style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        ...
        <ThemedText style={{ marginTop: spacing.md, opacity: 0.6 }}>Cargando grupo...</ThemedText>
      </ThemedView>
    );
  }
  ...
  const removeBackground = () => {
    ...
    showAlert({
      ...
      message: '¿Estás seguro de que quieres eliminar este fondo?',
      type: 'confirm',
      showCancelButton: true,
      onConfirm: () => {
        const saved = chatSettings.savedCustomBackgrounds || [];
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, flex: 1, cursor: 'pointer' }} onClick={() => setShowGroupInfo(true)}>
            ...
            <div style={{ minWidth: 0 }}>
              ...
              <ThemedText style={{ fontSize: 11, opacity: 0.7, display: 'block', color: colors.textSecondary }}>
                {group?.members?.length || 0} miembros • {(group?.members?.length || 0) > 1 ? 'Grupo activo' : 'Solo tú'}
              </ThemedText>
            </div>
          </div>

          <div style={{ display: 'flex', gap: 4, alignItems: 'center' }}>
            <button onClick={() => navigate(`/dm/group/${groupId}/call?type=audio`)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: colors.text, padding: 8, borderRadius: 8, display: 'flex' }}><Phone size={20} /></button>
            <button onClick={() => navigate(`/dm/group/${groupId}/call?type=video`)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: colors.text, padding: 8, borderRadius: 8, display: 'flex' }}><Video size={20} /></button>
          ...
          {showHeaderMenu && (
            <>
              ...
              <div style={{ position: 'absolute', top: 60, right: 20, zIndex: 100, backgroundColor: colors.card, border: `1px solid ${colors.border}`, borderRadius: 12, boxShadow: '0 4px 12px rgba(0,0,0,0.15)', minWidth: 200, padding: 6 }}>
                <button onClick={() => { setShowSettings(true); setShowHeaderMenu(false); }} style={menuItemStyle(colors)}><Type size={16} /> Personalizar chat</button>
                <div style={{ height: 1, backgroundColor: colors.border, margin: '4px' }} />
                <button onClick={() => { navigate(`/dm/group/${groupId}/call?type=audio`); setShowHeaderMenu(false); }} style={menuItemStyle(colors)}><Phone size={16} /> Llamada de audio</button>
                <button onClick={() => { navigate(`/dm/group/${groupId}/call?type=video`); setShowHeaderMenu(false); }} style={menuItemStyle(colors)}><Video size={16} /> Videollamada</button>
                <div style={{ height: 1, backgroundColor: colors.border, margin: '4px' }} />
                <button onClick={() => { setShowGroupInfo(true); setShowHeaderMenu(false); }} style={menuItemStyle(colors)}><Info size={16} /> Info del grupo</button>
                <div style={{ height: 1, backgroundColor: colors.border, margin: '4px' }} />
                <button onClick={handleLeaveGroup} style={{ ...menuItemStyle(colors), color: colors.danger }}><LogOut size={16} /> Salir del grupo</button>
              </div>
            </>
          )}
        </div>
        ...
        {isSearchOpen && (
          <div style={{ padding: `8px ${spacing.md}px`, backgroundColor: colors.backgroundSecondary, borderBottom: `1px solid ${colors.border}`, display: 'flex', alignItems: 'center', gap: spacing.sm, animation: 'slideDown 0.2s ease-out' }}>
            ...
            <input
              ...
              placeholder="Buscar en el chat de grupo..."
            ...
            <div style={{ display: 'flex', flexDirection: 'column' }}>
              {filteredMessages.length === 0 ? (
                <div style={{ padding: spacing.xl, textAlign: 'center', opacity: 0.6 }}>
                  <ThemedText>{searchQuery ? 'No se encontraron mensajes' : '¡Bienvenidos al grupo!'}</ThemedText>
                </div>
                ...
                <MessageBubble 
                  ...
                  onLongPress={() => {}} // Placeholder as long press isn't common on desktop, or can show a context menu
              ...
              {Object.values(chatThemes).map(t => {
                ...
                return (
                  <button
                    ...
                    style={{
                      aspectRatio: '1', borderRadius: 8, backgroundColor: (t as any).background,
