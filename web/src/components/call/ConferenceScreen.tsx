...
import {
  ...
  MoreHorizontal, ChevronUp, ChevronRight, Check, Maximize2, ExternalLink, Users, Presentation
} from 'lucide-react';
import {
  ICE_SERVERS,
  getConnectionId,
...
export default function ConferenceScreen({
    ...
    async function init() {
      ...
      try {
        ...
        try {
          ...
          setMediaError('No se detectó micrófono ni cámara. Comprueba los permisos del navegador.');
        }
      }
  ...
  return (
    ...
    >
              ...
              <div style={{ ...tileStyle('local'), ...(localSpeaking && { boxShadow: '0 0 0 2px #23a55a, 0 0 12px rgba(35,165,90,0.45)' }) }} onClick={onTileClick('local')}>
                ...
                {(callType === 'audio' || !camOn) && (
                  <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
                    ...
                    <p style={{ color: '#fff', fontSize: 13, fontWeight: 600, margin: '8px 0 2px' }}>Tú</p>
                  </div>
                )}
                {tileLabel('Tú')}
              </div>
      ...
      {isInitiator && pendingApprovals.length > 0 && !minimized && (
        ...
        <div style={{ backgroundColor: '#292b2f', padding: isMobile ? '10px 12px 24px' : '12px 20px 20px', flexShrink: 0, position: 'relative' }}>
          {showDevices && (
            <div style={{
              ...
              backgroundColor: '#111214', borderRadius: 8, zIndex: 30, boxShadow: '0 16px 32px rgba(0,0,0,0.8)'
            }}>
              <button className="dev-row" onClick={() => setSubPanel(p => p === 'input' ? null : 'input')}>
                ...
                <ChevronRight size={16} color="#72767d" style={{ flexShrink: 0 }} />
              </button>
          ...
          {showDevices && subPanel !== null && (
            <div style={{
              position: 'absolute',
              ...(isMobile ? { bottom: 'calc(100% + 8px)', left: 12, right: 12, width: 'auto' } : { bottom: 'calc(100% + 8px)', left: 'calc(50% + 148px)', width: 260 }),
              backgroundColor: '#111214', borderRadius: 8, zIndex: 31, boxShadow: '0 16px 32px rgba(0,0,0,0.8)',
          ...
          {showCamPicker && callType === 'video' && (
            <div style={{
              ...
              {devices.filter(d => d.kind === 'videoinput').map(d => (
                <button key={d.deviceId} className="dev-sub" onClick={() => changeVideoInput(d.deviceId)}>
