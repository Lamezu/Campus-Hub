...
function getPinExpiryText(pinnedUntil: string | null | undefined, t: (key: string, opts?: Record<string, string>) => string): string | null {
  if (!pinnedUntil) return null;
  const remaining = new Date(pinnedUntil).getTime() - Date.now();
  if (remaining <= 0) return t('campus.announcement.pin_expiry.expired');
  const days = Math.ceil(remaining / 86400000);
  if (days < 2) return t('campus.announcement.pin_expiry.today');
  if (days < 8) return t('campus.announcement.pin_expiry.days', { days: String(days) });
  return t('campus.announcement.pin_expiry.weeks', { weeks: String(Math.ceil(days / 7)) });
}
...
export default function AnnouncementDetail() {
  ...
  const handleDelete = async () => {
    if (window.confirm(t('campus.announcement.delete_confirm'))) {
  ...
  const handlePublishSocial = async () => {
    ...
    if (!window.confirm(t('campus.announcement.publish_social_confirm'))) return;
  ...
  const handleDeleteDocs = async () => {
    if (!id || !window.confirm(t('campus.announcement.delete_docs_confirm'))) return;
  ...
  if (loading) {
    return (
      <Layout title={t('campus.announcement.detail_title')}>
  ...
  if (!announcement) {
    return (
      <Layout title={t('campus.announcement.detail_title')}>
        <div style={{ textAlign: 'center', marginTop: 60, color: colors.textSecondary }}>{t('campus.announcement.not_found')}</div>
      </Layout>
    );
  }
  ...
  return (
    <Layout title="Anuncio">
      <div style={{ maxWidth: 680, margin: '0 auto' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '8px 16px', borderBottom: `1px solid ${colors.border}` }}>
          <button onClick={() => navigate(-1)} style={{ display: 'flex', alignItems: 'center', gap: 2, background: 'none', border: 'none', cursor: 'pointer', color: colors.primary, fontSize: 15 }}>
            ...
            {t('campus.announcement.board_back')}
          </button>
          {isAuthor && (
        ...
        <div style={{ padding: '8px 16px', borderBottom: `1px solid ${colors.border}` }}>
          <span style={{ fontSize: 12, fontWeight: 500, color: colors.textSecondary }}>
            {announcement.authorName} · {new Date(announcement.createdAt).toLocaleDateString(language || 'es', { day: 'numeric', month: 'long', year: 'numeric' })}
          </span>
        </div>
        ...
        <div style={{ padding: 16, display: 'flex', flexDirection: 'column', gap: 16 }}>
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
            ...
            {announcement.pinned && (
              <div style={{ display: 'flex', alignItems: 'center', gap: 4, padding: '4px 10px', borderRadius: 8, backgroundColor: colors.primary + '12' }}>
                ...
                <span style={{ fontSize: 11, fontWeight: 700, color: colors.primary }}>{pinExpiry ?? t('campus.pinned')}</span>
              </div>
            )}
          </div>
            ...
            <div style={{ display: 'flex', gap: 8 }}>
              {announcement.docsContent && (
                ...
                >
                  ...
                  {t('campus.announcement.view_docs_btn')}
                </button>
              )}
              {isAuthor && (
                <button
                ...
                >
                  {announcement.docsContent
                    ...
                    : <><AlignLeft size={15} strokeWidth={2} /> {t('campus.announcement.docs_add')}</>
                  }
                </button>
              )}
            </div>
          ...
          {linkedEvent ? (
            ...
            >
              <div style={{ display: 'flex', alignItems: 'center', gap: 4, padding: '4px 8px', borderRadius: 6, backgroundColor: EVENT_TYPE_CONFIG[linkedEvent.type].color + '20' }}>
                ...
                <span style={{ fontSize: 10, fontWeight: 700, letterSpacing: 0.3, color: EVENT_TYPE_CONFIG[linkedEvent.type].color }}>
                  {t('events.types.' + linkedEvent.type).toUpperCase()}
                </span>
              </div>
              <div style={{ flex: 1 }}>
                <div style={{ fontSize: 14, fontWeight: 700, color: colors.text }}>{linkedEvent.title}</div>
                <div style={{ fontSize: 12, color: colors.textSecondary, marginTop: 2 }}>
                  {new Date(linkedEvent.date).toLocaleDateString(language || 'es', { weekday: 'long', day: 'numeric', month: 'long' })}
            ...
            >
              ...
              {t('campus.announcement.link_event')}
            </button>
          ) : null}
          ...
          {isAuthor && (
            announcement.socialId ? (
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '8px 14px', borderRadius: 10, border: '1px solid #FF950040', backgroundColor: '#FF950012' }}>
                ...
                <span style={{ fontSize: 12, fontWeight: 600, color: '#FF9500' }}>{t('campus.announcement.already_published')}</span>
              </div>
            ) : (
              <button
              ...
              >
                ...
                {t('campus.announcement.publish_social')}
              </button>
            )
          )}
      ...
      {showEdit && (
        <div style={{ position: 'fixed', inset: 0, zIndex: 200, backgroundColor: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'flex-start', justifyContent: 'center', paddingTop: 100, paddingLeft: 16, paddingRight: 16, paddingBottom: 16 }}>
          <div className="animate-slide-up" style={{ backgroundColor: colors.background, borderRadius: 16, width: '100%', maxWidth: 560, maxHeight: 'calc(100vh - 124px)', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '14px 16px', borderBottom: `1px solid ${colors.border}` }}>
              ...
              <span style={{ fontSize: 16, fontWeight: 700, color: colors.text }}>{t('campus.edit_announcement')}</span>
              <button
            ...
            <div style={{ flex: 1, overflowY: 'auto', padding: 16, display: 'flex', flexDirection: 'column', gap: 14 }}>
              ...
              <input type="text" placeholder={t('campus.announcement.title_placeholder')} value={editForm.title} onChange={e => setEditForm(f => ({ ...f, title: e.target.value }))}
              ...
              <textarea placeholder={t('campus.announcement.content_placeholder')} value={editForm.content} onChange={e => setEditForm(f => ({ ...f, content: e.target.value }))} rows={5}
        ...
        <div style={{ position: 'fixed', inset: 0, zIndex: 9999, display: 'flex', flexDirection: 'column', backgroundColor: '#fff' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 16px', borderBottom: `1px solid ${colors.border}`, flexShrink: 0, backgroundColor: colors.background }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              ...
              <span style={{ fontSize: 15, fontWeight: 700, color: colors.text }}>{t('campus.announcement.docs_title')}</span>
            </div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
              ...
              {isAuthor && (
                ...
                >
                  <Pencil size={13} strokeWidth={2} /> {t('common.edit')}
                </button>
              )}
            </div>
          </div>
          ...
          {showDocsSearch && (
            <div style={{ padding: '8px 16px', borderBottom: `1px solid ${colors.border}`, backgroundColor: colors.background, flexShrink: 0 }}>
              <input
                type="text"
                placeholder={t('campus.announcement.docs_search_placeholder')}
        ...
        <div style={{ position: 'fixed', inset: 0, zIndex: 200, backgroundColor: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }}>
          <div style={{ backgroundColor: colors.background, borderRadius: 16, width: '100%', maxWidth: 480, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '14px 16px', borderBottom: `1px solid ${colors.border}` }}>
              ...
              <span style={{ fontSize: 16, fontWeight: 700, color: colors.text }}>{t('campus.announcement.link_event_title')}</span>
              <button onClick={handleCreateEvent} disabled={!eventForm.title.trim() || savingEvent}
                ...
                {savingEvent ? '...' : t('common.create')}
              </button>
            </div>
            <div style={{ padding: 16, display: 'flex', flexDirection: 'column', gap: 14 }}>
              <input type="text" placeholder={t('campus.announcement.event_name_placeholder')} value={eventForm.title} onChange={e => setEventForm(f => ({ ...f, title: e.target.value }))}
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
                {EVENT_TYPES.map(evtType => {
                  ...
                  const active = eventForm.type === evtType;
                  return (
                    <button key={evtType} onClick={() => setEventForm(f => ({ ...f, type: evtType }))}
